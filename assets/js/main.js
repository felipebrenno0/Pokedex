...
const displayPopup = (pokemon) => {
    ...
    <img id="pokemon-img" src="${imagem}"
             ...
             <h4>Base stats</h4>
             <div id="stats">
             <div class="bar-inner">${pokemon.stats.map((name_stats)=>`<p>${name_stats.stat.name}</p>`).join('')}</div>
             <div class="stat-desc">${pokemon.stats.map((base_stats)=>`<p>${base_stats.base_stat}</p>`).join('')}</div>
